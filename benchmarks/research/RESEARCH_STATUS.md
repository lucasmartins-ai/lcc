...
| Laya batching (probe) | 1 block/state: specialised evidence 0.85/0.91 vs tool+log noise 0.003–0.016; shipped evidence 0.21/0.03 vs noise 0.92. 5 blocks/state: everything 0.88–0.94, both checkpoints | small corpus blocks via `Agent.predict` | 10 blocks | laya-multilingual + specialised | n/a | 2026-09-23 | direct `laya` probe (`docs/LAYA.md` §8) | probe-level, one corpus; explains why the batch-8 default measures keep-all |
| Laya specialised (head-only RLCD, frozen encoder) — canonical corpora, batch 1 | small −40.95% / medium −59.92% / large −70.48% at 100% category recall (mechanical −26.18 / −57.58 / −65.45%); in-training held-out slice 400 items: evidence 0.963 vs noise 0.068, evidence kept 1.000 / noise dropped 0.944 @0.4 | corpora small/medium/large | 3 scales (train 8160 items) | specialised laya-multilingual (`~/Models/laya-lcc-relevance-v1`) | exact o200k | 2026-09-23 | `python3 run_laya_comparison.py --scales small medium large --batch-size 1 --laya-model ~/Models/laya-lcc-relevance-v1` | **memorisation, not generalisation**: corpus evidence is the factory's ten fixed sentences. Do not cite as a relevance capability; the ablation row below is the honest number |
| Laya specialised — judge ablation, batch 1 | precision 0.0, recall 0.0, false_drops 6/6, false_keeps 2/6, critical_recall 0.0, reduction 0.801 | ablation corpus (6 evidence + 6 noise, worded independently) | 12 blocks | specialised laya-multilingual | heuristic | 2026-09-23 | `python3 run_judge_ablation.py --judge laya --laya-model ~/Models/laya-lcc-relevance-v1 --batch-size 1` | out-of-distribution evidence is dropped; padding a block with filler makes it worse. This checkpoint is a research artifact, not a shippable default |
| Laya real session (Hermes, 66 841 tokens) | 0.0% reduction: 40/77 blocks above the 1024-token window (kept whole), 34 above threshold, 5 below threshold all kept by safety layers (`high_risk_conservative_retention`, `quoted_speech_present`, `semantic_sufficiency_restoration`) | real Hermes session `20260923_102552_4bcebf` | 77 blocks | shipped + specialised laya-multilingual | exact o200k | 2026-09-23 | `python3 laya_real_context_test.py --hermes <id>` | structural: the window, not the judgement, binds on sessions; vendor documents mmBERT as "1024 (up to 8k)" |
