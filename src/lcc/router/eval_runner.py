...
from lcc.agents.local_verifier import _format_valid
from lcc.router.router import LCCRouter, final_answer_to_dict
from lcc.router.schemas import EvaluationResult, FinalAnswer, TaskInput


def load_task(path: str | Path) -> tuple[TaskInput, dict[str, Any]]:
    """Load task input and original fixture mapping."""
...
def run_evaluation(cases_dir: str | Path, router: LCCRouter | None = None) -> dict[str, Any]:
    """Execute evaluation over all task JSON fixtures in a directory."""
    ...
    return {
        "note": "Local development proxy only; deterministic benchmark evaluation.",
...
def write_reports(report: dict[str, Any], output: str | Path) -> Path:
    """Save JSON and Markdown evaluation summaries."""
    out = Path(output)
