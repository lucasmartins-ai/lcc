"""Router and local agent data contracts.

These schemas define the interfaces for context preparation, hybrid routing decisions,
verification gates, local agents, and evaluation metrics.
"""
...
class VerificationDecision(StrEnum):
    """Local quality verifier outcome."""
...
class AgentHealth:
    """Diagnostic status for local agent backend."""
...
@dataclass(frozen=True)
class TaskInput:
    """Standard task input for routing and inference."""
...
@dataclass(frozen=True)
class LocalAnswer:
    """Answer produced by a local solver or agent."""
...
@dataclass(frozen=True)
class RemoteAnswer:
    """Answer produced by a remote cloud model or fallback."""
...
class PreparedContext:
    """Context and prompt prepared by LCC for a router step."""
